...
test("test_sha256", async () => {
  ...
  expect(hash).toBe("beb0d9e314ef4af12e6b8c48d2b6b205eed1245a78f8d9c99164fd6fcce0ed9c");
});
...
test("test_hdkey_xprv_from_mnemonic", async () => {
  ...
  const { xprv } = await tonClient.crypto.hdkey_xprv_from_mnemonic({
    phrase: "huge vivid crowd exhaust seek decline income copper envelope black bench dial",
  });
  expect(xprv).toBe(
...
test("test_hdkey_secret_from_xprv", async () => {
  ...
  expect(secret).toBe("01d62c5c294477431c86da4fb837af93a3ae1057e2e8add57b7ffca65f4d7825");
  // add error
});
...
test("test_hdkey_public_from_xprv", async () => {
  ...
  expect(pubkey).toBe("039ab9ebf7cca4f13cb02b7b342cbd0b5ebbf33ce51c2982a6fbdabaefc99dab21");
  // add error
});
...
test("test generate_random_bytes", async () => {
  const tonClient = new TonClient({ network: { server_address: "net.ton.dev" } });
  const { bytes } = await tonClient.crypto.generate_random_bytes({ length: 10 });
...
test("test convert_public_key_to_ton_safe_format", async () => {
  const tonClient = new TonClient({ network: { server_address: "net.ton.dev" } });
  const { ton_public_key } = await tonClient.crypto.convert_public_key_to_ton_safe_format({
    public_key: 'BCA335626726CF2E522D287B27E4FAFFF82D1D98615957DB8E224CB397B2EB67'
  });

  expect(ton_public_key).toBe('Pua8ozViZybPLlItKHsn5Pr_-C0dmGFZV9uOIkyzl7LrZ7gv');
});
...
test("test sign", async () => {
  ...
  expect(signed).toBe('0WbqQZtnXq0EHtN+hsXn98R1jX8ecDdMwKxKCSfEeytBQfl2QhBqMK6CnuRpcsg563pPykIPZvDdad24RKHzClRlc3QgTWVzc2FnZQ==');
  expect(signature).toBe('d166ea419b675ead041ed37e86c5e7f7c4758d7f1e70374cc0ac4a0927c47b2b4141f97642106a30ae829ee46972c839eb7a4fca420f66f0dd69ddb844a1f30a')

  const { unsigned: verified } = await tonClient.crypto.verify_signature({ public: key.public, signed });
...
test('test scrypt', async () => {
  ...
  const { key } = await tonClient.crypto.scrypt({ log_n: 10, r: 8, p: 16, dk_len: 64, password: password, salt: salt });

  expect(key).toBe('03bb240d2fbb1fd1caded3137d281325dd0cf68f37f2315ae7ef27553be64731061e2db7633c88176a44775e9f1d6efd71d9954a429ae03f150462c389799735');
});

test('test nacl_sign_keypair_from_secret_key', async () => {
  const tonClient = new TonClient({ network: { server_address: "net.ton.dev" } });
  const key = await tonClient.crypto.nacl_sign_keypair_from_secret_key({ secret: '61a7b6578976c8c3b3830e6e5fd2b5676bad3b5724653e025ff21ad94011e357' });

  expect(key.public).toBe('0c7bb0034436c904f9cd8273a39f129c7c24f75c19488b6b82d06558a07dcbb1');
});
...
test('test nacl_sign', async () => {
  ...
  const unsigned = btoa('Test Message');
  const secret = '56b6a77093d6fdf14e593f36275d872d75de5b341942376b2a08759f3cbae78f1869b7ef29d58026217e9cf163cbfbd0de889bdf1bf4daebf5433a312f5b8d6e';
  const { signed } = await tonClient.crypto.nacl_sign({
    secret,
    unsigned
  });

  expect(signed).toBe('+wz+QO6l1slgZS5s65BNqKcu4vz24FCJz4NSAxef9lu0jFfs8x3PzSZRC+pn5k8+aJi3xYMA3BQzglQmjK3hA1Rlc3QgTWVzc2FnZQ==');

  const { unsigned: opened } = await tonClient.crypto.nacl_sign_open({ public: '1869b7ef29d58026217e9cf163cbfbd0de889bdf1bf4daebf5433a312f5b8d6e', signed });

  expect(opened).toBe('VGVzdCBNZXNzYWdl');

  const { signature } = await tonClient.crypto.nacl_sign_detached({ secret, unsigned });

  expect(signature).toBe('fb0cfe40eea5d6c960652e6ceb904da8a72ee2fcf6e05089cf835203179ff65bb48c57ecf31dcfcd26510bea67e64f3e6898b7c58300dc14338254268cade103');
});
...
test('test nacl_box_keypair_from_secret_key', async () => {
  const tonClient = new TonClient({ network: { server_address: "net.ton.dev" } });
  const key = await tonClient.crypto.nacl_box_keypair_from_secret_key({ secret: '61a7b6578976c8c3b3830e6e5fd2b5676bad3b5724653e025ff21ad94011e357' });

  expect(key.public).toBe('9c5816d3cf6bf55fa028dfaa7be09ad24e38a36fe9b8f43278ef0ada15c3ba0e');
});
...
test('test nacl_secret_box', async () => {
  ...
  const key = '8f68445b4e78c000fe4d6b7fc826879c1e63e3118379219a754ae66327764bd8';
...
test('test mnemonic_from_random', async () => {
  ...
  const { phrase } = await tonClient.crypto.mnemonic_from_random({ word_count: 15, dictionary: 1 });
...
test('test mnemonic_from_entropy', async () => {
  ...
  const { phrase } = await tonClient.crypto.mnemonic_from_entropy({ entropy: '00112233445566778899AABBCCDDEEFF' });
...
test('test mnemonic_derive_sign_keys', async () => {
  ...
  const { ton_public_key } = await tonClient.crypto.convert_public_key_to_ton_safe_format({ public_key: key.public });

  expect(ton_public_key).toBe('PuZhw8W5ejPJwKA68RL7sn4_RNmeH4BIU_mEK7em5d4_-cIx');
});
